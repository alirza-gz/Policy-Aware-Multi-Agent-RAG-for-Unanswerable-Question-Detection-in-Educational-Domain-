...
# When these environment variables are set (e.g. in the Docker Compose
# deployment) the gateway calls each agent as an independent HTTP service.
# When unset, it falls back to running the agents in-process, so local runs
# and the evaluation harness keep working without any services.
RETRIEVER_URL = os.getenv("RETRIEVER_URL")
REASONING_URL = os.getenv("REASONING_URL")
GOVERNANCE_URL = os.getenv("GOVERNANCE_URL")
_SERVICE_TIMEOUT = httpx.Timeout(30.0, read=300.0)


async def do_retrieve(query: str, top_k: int):
    """Return (passages, retriever_confidence) via HTTP service or in-process."""
    if RETRIEVER_URL:
        async with httpx.AsyncClient(timeout=_SERVICE_TIMEOUT) as client:
            ...
            data = resp.json()
        return data.get("passages", []), float(data.get("retriever_confidence", 0.0))

    passages = get_retriever().retrieve(query, top_k=top_k)
    retriever_confidence = max((p.get("score", 0.0) for p in passages), default=0.0)
    return passages, float(retriever_confidence)


async def do_reason(query: str, passages: list) -> Dict:
    if REASONING_URL:
        async with httpx.AsyncClient(timeout=_SERVICE_TIMEOUT) as client:
            resp = await client.post(
                f"{REASONING_URL}/reason", json={"query": query, "passages": passages}
            )
            resp.raise_for_status()
            return resp.json()
    return await get_reasoner().reason(query, passages)


async def do_decide(reasoning_result: Dict, retriever_confidence: float) -> Dict:
    if GOVERNANCE_URL:
        async with httpx.AsyncClient(timeout=_SERVICE_TIMEOUT) as client:
            resp = await client.post(
                f"{GOVERNANCE_URL}/decide",
                json={
            ...
            resp.raise_for_status()
            return resp.json()
    return get_governor().decide(reasoning_result, retriever_confidence=retriever_confidence)

class QueryRequest(BaseModel):
    query: str
    top_k: int = 5
    # Override the global governance switch per request. When explicitly set to
    # False the pipeline runs as a plain RAG baseline (retriever + reasoner only)
    # so the policy-aware system can be compared against an ablation baseline.
...
async def query(req: QueryRequest, session_id: Optional[str] = Header(default="default")):
    ...
    # 1) Retrieve (via microservice or in-process)
    try:
        passages, retriever_confidence = await do_retrieve(q, top_k)
    except Exception as e:
    ...
    # 2) Reason (via microservice or in-process)
    try:
        reasoning_result = await do_reason(q, passages)
        answer = reasoning_result.get("answer", "")
        trace = reasoning_result.get("trace", [])
        confidence = float(reasoning_result.get("confidence", 0.0))
        is_answerable = bool(reasoning_result.get("is_answerable", bool(answer)))
    except Exception as e:
    ...
    # Resolve whether the governance agent participates in this request.
    if req.governance_enabled is not None:
        governance_enabled = req.governance_enabled
    else:
        governance_enabled = bool(getattr(Config, "GOVERNANCE_ENABLED", True))

    # 3) Govern (three-way decision) OR run as a plain RAG baseline.
    if governance_enabled:
        try:
            decision = await do_decide(reasoning_result, retriever_confidence)
        except Exception as e:
            _agent_error_counts["governance"] += 1
            _agent_errors["governance"].append(f"{datetime.now()}: {str(e)}")
            logger.error(f"Governance error: {e}")
            # Fail open to an answer rather than dropping the query.
            decision = {
                "action": "ANSWER",
                "approved": True,
                "reason": f"governance_error: {e}",
                "reasons": [f"governance_error: {e}"],
                "final_answer": answer,
                "redacted_answer": answer,
                "clarification_question": "",
            }
    else:
        # Baseline: no governance agent, always emit the raw model answer.
        decision = {
            "action": "ANSWER",
            "approved": True,
            "reason": "governance_disabled_baseline",
            "reasons": ["governance_disabled_baseline"],
            "final_answer": answer,
            "redacted_answer": answer,
            "clarification_question": "",
        }
    ...
    final_answer = decision.get("final_answer", decision.get("redacted_answer", answer))
