            ...
            with st.spinner("🔍 Processing your query..."):
                try:
                    ...
                    if resp.status_code == 200:
                        ...
                        action_badges = {
                            "ANSWER": ("✅ ANSWER", st.success),
                            "CLARIFY": ("❓ CLARIFY", st.warning),
                            "ABSTAIN": ("🚫 ABSTAIN", st.error),
                        }
                        label, badge_fn = action_badges.get(action, ("✅ ANSWER", st.success))
                        badge_fn(f"Governance decision: {label}")

                        if action == "CLARIFY":
                            st.write("### Clarification needed:")
                            st.markdown(f"**{data.get('clarification_question') or data['answer']}**")
                        else:
                            st.write("### Answer:")
                        ...
                        st.caption(f"Model judged answerable: {data.get('is_answerable')}")

                        # --- Governance Section ---
                        with st.expander("🛡️ Governance Details"):
                            gov = data.get("governance", {})
                            st.write(f"**Action:** {gov.get('action', action)}")
                            st.write(f"**Governance enabled:** {gov.get('enabled')}")
                            st.write(f"**Reason:** {gov.get('reason')}")
                            st.json(gov)
