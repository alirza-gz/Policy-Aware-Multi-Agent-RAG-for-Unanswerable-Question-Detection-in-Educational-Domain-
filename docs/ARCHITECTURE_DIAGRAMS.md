...
> **Research focus:** Policy-aware, multi-agent RAG for **unanswerable question
> detection** in the educational domain. The governance agent chooses one of
> three actions per query - **ANSWER**, **CLARIFY**, or **ABSTAIN** - so the
> system avoids guessed or misleading answers.

## Policy-Aware Decision Flow

```mermaid
flowchart TD
    Q[User Query] --> RET[Retriever Agent - FAISS]
    ...
    GOV --> S{Safety policy violated?}
    S -->|yes| ABS[ABSTAIN - blocked]
    S -->|no| EV{Answerable and evidence sufficient?}
    EV -->|no| ABS2[ABSTAIN - no reliable evidence]
    EV -->|yes| AMB{Ambiguous or mid-band confidence?}
    AMB -->|yes| CLR[CLARIFY - ask a question]
    AMB -->|no| ANS[ANSWER - grounded, PII redacted]
```

Decision signals and thresholds live in `config.yml` under `GOVERNANCE`
(`retriever_abstain_below`, `reasoner_abstain_below`, `clarify_band`). Setting
`GOVERNANCE_ENABLED: false` disables the agent for the ablation baseline.

## Microservice Deployment

Under Docker Compose each agent runs as an independent, separately scalable
service. The gateway routes over HTTP; if the `*_URL` environment variables are
unset it falls back to in-process agents (used by local runs and the eval harness).

```mermaid
flowchart LR
    UI[Streamlit UI / API client] --> GW["rag_gateway :8010"]
    GW -->|"RETRIEVER_URL"| RS["retriever_service :8011"]
    GW -->|"REASONING_URL"| RE["reasoning_service :8012"]
    GW -->|"GOVERNANCE_URL"| GS["governance_service :8013"]
    RE --> OL["ollama :11434"]
    GW --> PG["postgres :5432 (audit log)"]
```
